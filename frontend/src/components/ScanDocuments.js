...
import { ArrowLeft, Camera, Upload, FileText, Trash2, Eye, Download, ScanLine, Save, Plus, X, FilePlus, Edit, RotateCcw } from 'lucide-react';
...
const ScanDocuments = () => {
  ...
  const [showPdfNameDialog, setShowPdfNameDialog] = useState(false);
  ...
  // Initiate save as PDF
  const handleInitiateSavePdf = () => {
    if (!editedImage) return;
    
    const finalPages = [...collectedPages, {
      id: Date.now(),
      image: editedImage,
      thumbnail: editedImage
    }];
    
    setCollectedPages(finalPages);
    setShowEditDialog(false);
    
    setPdfName(generatePdfName());
    setShowPdfNameDialog(true);
  };
  ...
  const handleAddPageToPdf = () => {
    // Close PDF editor temporarily to add new page
    setShowPdfEditorDialog(false);
    
    // Show options: camera or upload
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white pb-32">
      ...
      {collectedPages.length > 0 && !showPdfEditorDialog && (
        <div className="fixed bottom-0 left-0 right-0 bg-slate-800 border-t border-slate-700 p-4 z-40">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm text-slate-300">Collected Pages: {collectedPages.length}</span>
          </div>
        ...
        <DialogContent className="bg-slate-800 border-slate-700 max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <Button
                onClick={() => {
                  setPdfName(scannedDocuments.find(d => d.id === editingPdfId)?.name.replace('.pdf', '') || generatePdfName());
                  setShowPdfNameDialog(true);
                }}
      ...
      {/* PDF Naming Dialog */}
      <Dialog open={showPdfNameDialog} onOpenChange={setShowPdfNameDialog}>
        <DialogContent className="bg-slate-800 border-slate-700 max-w-md">
          <DialogHeader>
            <DialogTitle className="text-white flex items-center gap-2">
              <FilePlus className="w-5 h-5 text-green-400" />
              {editingPdfId ? 'Update PDF' : 'Save PDF Document'}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">
                Total pages: {collectedPages.length}
              </p>
            </div>
            <div className="flex gap-2">
              <Button
              ...
              >
                <Save className="w-4 h-4 mr-2" />
                {editingPdfId ? 'Update PDF' : 'Save PDF'}
              </Button>
              <Button
                onClick={() => setShowPdfNameDialog(false)}
                variant="outline"
                className="border-slate-600 text-slate-200"
              >
                Cancel
