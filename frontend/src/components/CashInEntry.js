      ...
      <Dialog open={showBillModal} onOpenChange={setShowBillModal}>
        <DialogContent className="bg-slate-800 border-slate-700 max-w-xl w-full mx-auto my-0 h-screen overflow-y-auto p-2">
          ...
          {selectedSlotForBill !== null && (
            <div className="space-y-0.5 bg-white p-1 rounded text-xs">
              {/* GST Invoice Header */}
              <div className="text-center border border-black p-0.5">
                <div className="text-xs font-bold text-black">{activeBusiness?.name || 'BUSINESS'}</div>
                <div className="text-xs text-black flex justify-center gap-2">
                  {activeBusiness?.phone && <span>{activeBusiness.phone}</span>}
                  {activeBusiness?.gst && <span>{activeBusiness.gst}</span>}
                </div>
              </div>
              ...
              <div className="border border-black text-xs">
                ...
                <div className="border border-black text-xs">
                  <div className="bg-white p-0.5">
                    <div className="flex justify-between text-black">
                      <span>Taxable:</span>
                      <span>₹{parseFloat(slots[selectedSlotForBill]?.amount || 0).toFixed(2)}</span>
                    </div>
                    {(() => {
                  ...
                  <div className="bg-gray-100 border-t border-black p-0.5 font-bold text-black">
                ...
                <div className="border border-black p-1 bg-white">
                  <div className="flex justify-between items-center">
                    <div className="font-semibold text-black text-xs">Terms:</div>
                    {!isEditingTerms ? (
                      <Button
                        onClick={() => {
                          setTempTermsText(termsText);
                        ...
                        variant="ghost"
                        size="sm"
                        className="text-xs h-4 px-1 text-blue-600 hover:text-blue-700"
                      >
                        Edit
                      </Button>
                    ) : (
                      <div className="flex gap-0.5">
                        <Button onClick={saveTerms} size="sm" className="h-4 px-1 text-xs bg-green-600 hover:bg-green-700 text-white">
                          Save
                        </Button>
                        <Button onClick={cancelTermsEdit} variant="ghost" size="sm" className="h-4 px-1 text-xs text-gray-600">
                          Cancel
                        </Button>
                      </div>
                    )}
                  ...
                  <div className="text-xs text-black">
                    {isEditingTerms ? (
                      <textarea
                        ...
                        className="w-full bg-white border border-gray-300 text-black text-xs p-1 rounded min-h-[40px] focus:outline-none focus:border-blue-500"
                        placeholder="Enter terms..."
                      />
                    ) : (
                      <div className="whitespace-pre-line text-xs line-clamp-2">
                        {termsText}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="text-right mt-1">
                    <div className="text-xs text-black">For {activeBusiness?.name || 'BUSINESS'}</div>
                    <div className="mt-2 text-xs text-black border-t border-black pt-0.5 inline-block">Auth. Sign</div>
                  </div>
                </div>
