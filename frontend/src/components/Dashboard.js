...
import { Home, UserCircle, ShieldCheck, Users, Truck, Star, ShoppingCart, Zap, Building, Coins, Receipt, Package, PieChart, BarChart3, Gift, MessageCircle, Send, Plus, Minus, Shield, Settings, ChevronDown, ChevronLeft, ChevronRight, CheckSquare, Share2, Copy, ArrowLeft, ScanLine, FileBarChart, Landmark, Fuel, Download, Upload, TrendingUp, ClipboardList, ArrowDownRight, ArrowUpRight, Calendar, Calculator, Edit2, X, CreditCard } from 'lucide-react';
...
export default function Dashboard({ user, onLogout }) {
  ...
  const [showTopFloatingPanel, setShowTopFloatingPanel] = useState(false);
  const [showBottomFloatingPanel, setShowBottomFloatingPanel] = useState(false);
  
  // Swipe detection states for tabs
...
It's completely free to try!`;
  ...
  const businessTiles = [
    ...
    { name: 'Company', subtitle: 'Purchase', icon: ShoppingCart, iconColor: 'text-orange-400', permissions: ['expenses_manage'], showAddButton: true },
    { name: 'Stock', subtitle: 'Management', icon: Package, iconColor: 'text-orange-400', permissions: ['inventory_manage', 'inventory_view'], showAddButton: true },
    { name: 'Staff', subtitle: '', icon: Users, iconColor: 'text-purple-400', permissions: ['staff_manage'], showAddButton: true },
    { name: 'Challan Gate Pass', subtitle: '', icon: ClipboardList, iconColor: 'text-cyan-400', permissions: ['reports_view'], showAddButton: true },
    { name: 'Bills &', subtitle: 'Invoices', icon: FileBarChart, iconColor: 'text-yellow-400', permissions: ['reports_view'], showAddButton: true },
  ];
            ...
            >
              <div className="grid grid-cols-3 gap-x-3 gap-y-0 px-4 pb-20 min-h-[400px]">
                {getTilesForTab(tab).map((tile, idx) => {
                  ...
                  return (
                    <Card key={idx} onClick={() => handleTileClick(tile)} className="bg-slate-700/80 border border-slate-600 hover:bg-slate-600 transition-all duration-200 cursor-pointer shadow-xl aspect-square flex items-center justify-center m-0">
      ...
      {!showProfileDialog && (
        <div className="fixed top-1/2 right-1 transform -translate-y-1/2 z-50">
          ...
          <Button
            ...
            className={`w-4 h-24 rounded-md bg-slate-700/90 hover:bg-slate-600 border border-slate-600 shadow-lg flex items-center justify-start pl-0.5 backdrop-blur-sm transition-all duration-200 ${
