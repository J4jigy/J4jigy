...
import { Home, UserCircle, ShieldCheck, Users, Truck, Star, ShoppingCart, Zap, Building, Coins, Receipt, Package, PieChart, BarChart3, Gift, MessageCircle, Send, Plus, Minus, Shield, Settings, ChevronDown, CheckSquare, Share2, Copy, ArrowLeft, ScanLine, FileBarChart, Landmark, Fuel, Download, Upload, TrendingUp, ClipboardList, ArrowDownRight, ArrowUpRight, Calendar, Calculator, Edit2 } from 'lucide-react';
...
It's completely free to try!`;
  ...
  const businessTiles = [
    { name: 'Customers', subtitle: 'Debtors (देनदार)', icon: Users, iconColor: 'text-green-400', permissions: ['customers_manage', 'customers_view'] },
    { name: 'Suppliers', subtitle: 'Creditors (लेनदार)', icon: Truck, iconColor: 'text-indigo-400', permissions: ['suppliers_manage', 'suppliers_view'] },
            ...
            <TabsContent key={tab} value={tab} className="mt-0">
              <div className="grid grid-cols-3 gap-3 px-4">
                {getTilesForTab(tab).map((tile, idx) => {
                  ...
                  // Increase size specifically for Customers and Suppliers icons
                  const iconSize = (tile.name === 'Customers' || tile.name === 'Suppliers') ? 'w-14 h-14' : 'w-12 h-12';
                  return (
                    <Card key={idx} onClick={() => handleTileClick(tile)} className="bg-slate-700/80 border border-slate-600 hover:bg-slate-600 transition-all duration-200 cursor-pointer shadow-xl aspect-square flex items-center justify-center">
                      <CardContent className="p-3 flex flex-col items-center justify-center text-center w-full h-full">
                        <IconComponent className={`${iconSize} mb-2 ${tile.iconColor}`} />
